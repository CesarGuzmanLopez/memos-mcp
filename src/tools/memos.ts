...
export const registerMemoTools = (
  ...
  server.registerTool(
    "create",
    {
      description: "Create a memo with markdown content. Use #hashtags for tags. CRITICAL LIMIT: Each memo MUST NOT exceed 13,000 characters. The Memos server enforces a hard limit of ~18K but to avoid errors always stay under 13K. If you need to write more, split into multiple memos with a shared tag (e.g. #project/my-post) and add part numbers.",
      inputSchema: {
        content: z.string().min(1).max(13000).describe("Markdown content with #tags. CRITICAL: MUST NOT exceed 13,000 chars. Server hard limit is ~18K but stay under 13K to avoid errors. Split longer content into multiple memos sharing a #parent-tag."),
  ...
  server.registerTool(
    "update",
    {
      ...
      inputSchema: {
        id: z.string().min(1).describe("Memo ID or UID"),
        content: z.string().max(13000).optional().describe("New markdown content. CRITICAL: MUST NOT exceed 13,000 chars. Server hard limit is ~18K but stay under 13K."),
